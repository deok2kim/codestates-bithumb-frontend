        ...
        <tbody>
          {data.map((info, index) => (
            <tr key={index}>
              ...
              <td>{parseInt(info.contPrice).toLocaleString('ko-KR', {maximumFractionDigits: 4} )}</td>
							{}
              <td><ColorText buySellGb={info.buySellGb === '1' ? 'blue' : 'red'}>{parseFloat(info.contQty).toFixed(4)} {symbol.split("_")[0]}</ColorText></td>
            </tr>
          ))}
        </tbody>
      </Table>
